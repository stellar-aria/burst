...
struct EERef {
  ...
  EERef(const size_t index) : index(index) {}
  ...
  size_t index; // Index of current EEPROM cell.
};
...
struct EEPROMClass {
  ...
  EERef operator[](const size_t idx) { return idx; }
  uint8_t read(size_t idx) { return EERef(idx); }
  void write(size_t idx, uint8_t val) { (EERef(idx)) = val; }
  void update(size_t idx, uint8_t val) { EERef(idx).update(val); }
  ...
  template <typename T>
  T &get(size_t idx, T &t) {
    EEPtr e = idx;
    uint8_t *ptr = (uint8_t *)&t;
    for (size_t count = sizeof(T); count; --count, ++e) {
      *ptr++ = *e;
    }
    return t;
  }
  ...
  template <typename T>
  const T &put(size_t idx, const T &t) {
    EEPtr e = idx;
